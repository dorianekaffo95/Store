...
// Views
if ($show_all_counters || jardiwinery_strpos($post_options['counters'], 'views')!==false && function_exists('jardiwinery_reviews_theme_setup')) {
	?>
	<<?php jardiwinery_show_layout($counters_tag); ?> class="post_counters_item post_counters_views icon-eye" title="<?php echo esc_attr( sprintf(esc_attr__('Views - %s', 'jardiwinery'), $post_data['post_views']) ); ?>" href="<?php echo esc_url($post_data['post_link']); ?>"><span class="post_counters_number"><?php echo esc_attr($post_data['post_views']); ?></span><?php if (jardiwinery_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Views', 'jardiwinery'); ?></<?php jardiwinery_show_layout($counters_tag); ?>>
	<?php
}
...
if ($show_all_counters || jardiwinery_strpos($post_options['counters'], 'comments')!==false) {
	?>
	<a class="post_counters_item post_counters_comments" title="<?php echo esc_attr( sprintf(esc_attr__('Comments - %s', 'jardiwinery'), $post_data['post_comments']) ); ?>" href="<?php echo esc_url($post_data['post_comments_link']); ?>"><span class="post_counters_number"><?php echo esc_attr($post_data['post_comments']); echo esc_html__(' Comment(s)', 'jardiwinery');?></span><?php if (jardiwinery_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Comments', 'jardiwinery'); ?></a>
	<?php 
}
 
// Rating
$rating = $post_data['post_reviews_'.(jardiwinery_get_theme_option('reviews_first')=='author' ? 'author' : 'users')];
if ($rating > 0 && ($show_all_counters || jardiwinery_strpos($post_options['counters'], 'rating')!==false) && function_exists('jardiwinery_reviews_theme_setup')) {
	?>
	<<?php jardiwinery_show_layout($counters_tag); ?> class="post_counters_item post_counters_rating icon-star" title="<?php echo esc_attr( sprintf(esc_attr__('Rating - %s', 'jardiwinery'), $rating) ); ?>" href="<?php echo esc_url($post_data['post_link']); ?>"><span class="post_counters_number"><?php echo esc_attr($rating); ?></span></<?php jardiwinery_show_layout($counters_tag); ?>>
	<?php
}

// Likes
if ($show_all_counters || jardiwinery_strpos($post_options['counters'], 'likes')!==false && function_exists('jardiwinery_reviews_theme_setup')) {
	// Load core messages
