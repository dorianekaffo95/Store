...
if ( !function_exists( 'jardiwinery_core_frontend_scripts' ) ) {
	function jardiwinery_core_frontend_scripts() {
		...
		wp_enqueue_script( 'modernizr', jardiwinery_get_file_url('js/photostack/modernizr.min.js'), array(), null, false );
		...
		if (is_array($theme_fonts) && count($theme_fonts) > 0) {
            ...
            if ($google_fonts) {
                ...
                */
                $google_fonts_enabled = ( 'off' !== esc_html_x( 'on', 'Google fonts: on or off', 'jardiwinery' ) );
                if ( $google_fonts_enabled ) {
                    wp_enqueue_style( 'jardiwinery-font-google-fonts-style', add_query_arg( 'family', $google_fonts . '&subset=' . jardiwinery_get_theme_option('fonts_subset'), "//fonts.googleapis.com/css" ), array(), null );
		...
		wp_enqueue_style( 'fontello-style',  jardiwinery_get_file_url('css/fontello/css/fontello.css'),  array(), null);
		...
		if (jardiwinery_get_theme_option('menu_slider')=='yes') {
			wp_enqueue_script( 'slidemenu-script', jardiwinery_get_file_url('js/jquery.slidemenu.js'), array('jquery'), null, true );
		}

		if ( function_exists('jardiwinery_reviews_theme_setup') && is_single() && jardiwinery_get_custom_option('show_reviews')=='yes' ) {
		...
		if (jardiwinery_get_custom_option('show_video_bg') == 'yes' && jardiwinery_get_custom_option('video_bg_youtube_code') != '') {
			wp_enqueue_script( 'video-bg-script', jardiwinery_get_file_url('js/jquery.tubular.1.0.js'), array('jquery'), null, true );
		}
...
if ( !function_exists( 'jardiwinery_enqueue_slider' ) ) {
	function jardiwinery_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			wp_enqueue_style(  'swiperslider-style', 			jardiwinery_get_file_url('js/swiper/swiper.css'), array(), null );
			// jQuery version of Swiper conflict with Revolution Slider!!! Use DOM version
			wp_enqueue_script( 'swiperslider-script', 			jardiwinery_get_file_url('js/swiper/swiper.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'jardiwinery_enqueue_polaroid' ) ) {
	function jardiwinery_enqueue_polaroid() {
		wp_enqueue_style(  'polaroid-style', 	jardiwinery_get_file_url('js/photostack/component.css'), array(), null );
		wp_enqueue_script( 'classie-script',		jardiwinery_get_file_url('js/photostack/classie.js'), array(), null, true );
		wp_enqueue_script( 'polaroid-script',	jardiwinery_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'jardiwinery_enqueue_diagram' ) ) {
	function jardiwinery_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') wp_enqueue_script( 'diagram-chart-script',	jardiwinery_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') wp_enqueue_script( 'diagram-raphael-script',	jardiwinery_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if ( !function_exists( 'jardiwinery_enqueue_popup' ) ) {
	function jardiwinery_enqueue_popup($engine='') {
		if ($engine=='pretty' || (empty($engine) && jardiwinery_get_theme_option('popup_engine')=='pretty')) {
			wp_enqueue_style(  'prettyphoto-style',	jardiwinery_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			wp_enqueue_script( 'prettyphoto-script',	jardiwinery_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else if ($engine=='magnific' || (empty($engine) && jardiwinery_get_theme_option('popup_engine')=='magnific')) {
			wp_enqueue_style(  'magnific-style',	jardiwinery_get_file_url('js/magnific/magnific-popup.css'), array(), null );
			wp_enqueue_script( 'magnific-script',jardiwinery_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
...
if (!function_exists('jardiwinery_core_add_scripts_inline')) {
	function jardiwinery_core_add_scripts_inline() {
		...
		$ajax_search_min_length = empty(jardiwinery_get_theme_option('ajax_search_min_length')) ? 3 : jardiwinery_get_theme_option('ajax_search_min_length');
        ...
        echo '<'.'script'.'>'
			...
			. "JARDIWINERY_STORAGE['ajax_search_min_length']	= " . min(3, $ajax_search_min_length) . ";"
...
if ( !function_exists( 'jardiwinery_callback_frontend_editor_save' ) ) {
	function jardiwinery_callback_frontend_editor_save() {
		...
		parse_str(jardiwinery_get_value_gpc('data'), $output);
...
if ( !function_exists( 'jardiwinery_callback_frontend_editor_delete' ) ) {
	function jardiwinery_callback_frontend_editor_delete() {
		...
		$post_id = jardiwinery_get_value_gpc('post_id');
